...
uint16_t	ANARegs[MB_ANA_REGS];	//Registros para "Analog Input Registers"
...
ModbusDomoboard::ModbusDomoboard():DomoBoard() {
	...
	*(BTN_OPT.mbReg) = BTN_OPT.Sensor->valor;	//Actualizamos el registro ModBus con el estado del sensor
	...
	POT1.mbReg 	= &ANARegs[MB_POT1];
	*(POT1.mbReg) = POT1.Sensor->valor; // Valor inicial
	listmbSensors.push(&POT1);

	POT2.Sensor 	= &(DomoBoard::POT2);
	POT2.mbReg 	= &ANARegs[MB_POT2];
	*(POT2.mbReg) = POT2.Sensor->valor; // Valor inicial
