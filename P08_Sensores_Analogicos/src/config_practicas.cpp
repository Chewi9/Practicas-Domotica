...
void Config_P8_Potenciometros() {
	Config_P7_SensorMovimiento(); // Partimos de la configuración del ejercicio anterior
	mbDomoboard.POT1.Sensor->Activo = true;		// Habilitamos sensor POT1
	mbDomoboard.POT2.Sensor->Activo = true;		// Habilitamos sensor POT2
}
...
void SelectionConfiguration(uint8_t selConf){
	...
	switch(selConf){
		...
		case P8_POTENCIOMETROS:
			DEBUGLNF("P08 Seleccionada --> Sensores Potenciometros");
			Config_P8_Potenciometros();
			break;	
