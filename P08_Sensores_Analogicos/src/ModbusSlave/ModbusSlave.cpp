...
enum { 
	...
	FC_READ_ANALOG_INPUTS = 0x04 //Read Analog Input Registers de entrada
};
...
const unsigned char fsupported[] = { FC_READ_COILS, FC_READ_REG_DI, FC_READ_REG_AO, FC_WRITE_COIL,
		                             FC_WRITE_REG, FC_WRITE_REGS, FC_READ_ANALOG_INPUTS };
...
void ModbusSlave::set_BankRegisters(uint16_t *Regs, uint16_t lengthRegs, BankType type) {
	switch(type) {
		...
		case BANK_HOLDING_REGS:
			...
			N_ARegs = lengthRegs;
			break;
		case BANK_ANALOG_INPUTS:
			ANARegs = Regs;
			N_ANARegs = lengthRegs;
			break;	
	}
}
...
int ModbusSlave::validate_request(unsigned char *data, unsigned char length)
{
	...
	if(FC_READ_ANALOG_INPUTS == data[FUNC]) {
		regs_size = N_ANARegs;
		max_regs_num = MAX_READ_REGS;
	}
...
int ModbusSlave::read_input_registers(unsigned int start_addr, unsigned char reg_count, uint16_t *regs){
	unsigned char function = FC_READ_ANALOG_INPUTS; // 0x04
	int packet_size = 3;
	int status;
	unsigned int i;
	unsigned char packet[MAX_MESSAGE_LENGTH];
...
int ModbusSlave::update()
{
	...
	case FC_READ_ANALOG_INPUTS: // Formato de respuesta identico al 0x03, apuntamos a banco ANARegs en vez de ARegs
		return read_input_registers(start_addr,query[REGS_L],ANARegs);	
