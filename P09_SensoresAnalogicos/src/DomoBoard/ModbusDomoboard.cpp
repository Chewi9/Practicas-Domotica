...
ModbusDomoboard::ModbusDomoboard():DomoBoard() {
	...
	*(BTN_OPT.mbReg) = BTN_OPT.Sensor->valor;	//Actualizamos el registro ModBus con el estado del sensor
	...
	FOTORES.Sensor = &(DomoBoard::FOTORES);
	FOTORES.mbReg = &Iregs[MB_FOTORES];

	listmbSensors.push(&FOTORES);

	FOTOTR.Sensor = &(DomoBoard::FOTOTR);
	FOTOTR.mbReg = &Iregs[MB_FOTOTR];

	listmbSensors.push(&FOTOTR);
	...
	Aregs[MB_FOTORES_HI] = 500;
	Aregs[MB_FOTORES_LO] = 400;
	Aregs[MB_FOTOTRans] = 300;
...
void ModbusDomoboard::leerSensor(TpmbSensor Sensor){
	...
	// Temperatura
	if(Sensor->mbReg == &Iregs[MB_TEMP]) {
		int reg = Sensor->Sensor->valor;
		int t = (int)(((reg * 0.004882812f) - 0.5f) * 100.0f);

		*(Sensor->mbReg) = (uint16_t)(t > 0 ? t : 0); // Evitamos valores negativos
		return;
	}

	// Fotores y FotoTr
	if(Sensor->mbReg == &Iregs[MB_FOTORES] || Sensor->mbReg == &Iregs[MB_FOTOTR]) {
		*(Sensor->mbReg) = Sensor->Sensor->valor; // Actualizamos el registro con el valor leído del sensor
		if (Sensor->mbSensorEvent != NULL) {
			Sensor->mbSensorEvent(Sensor); // Lanzamos el evento asociado al sensor
		}
		return;
	}
