...
DomoBoard::DomoBoard()
{
	...
	FOTORES.pin 		= FotoRes;
	FOTORES.Activo 		= true;
	FOTORES.eSensor 	= S_ANALOGICO;
	FOTORES.name 		= F("Fotoresistencia");

	FOTOTR.pin 			= FotoTr;
	FOTOTR.Activo 		= true;
	FOTOTR.eSensor 		= S_ANALOGICO;
	FOTOTR.name 		= F("FotoTransistor");

	TEMP.pin 			= TEMP_P;
	TEMP.Activo 		= true;
	TEMP.eSensor 		= S_ANALOGICO;
	TEMP.name 			= F("Sensor de Temperatura");
	...
	listSensors.push(&FOTORES);
	listSensors.push(&FOTOTR);
