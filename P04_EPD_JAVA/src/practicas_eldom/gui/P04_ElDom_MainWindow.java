...
public class P04_ElDom_MainWindow {
...
//	private 	SerialPort 				serialPort;	
	...
	public P04_ElDom_MainWindow() {		
      ...
      //display date time to status bar
		...
		JLabel tituloSeleccion = new JLabel("Seleccionar apartado de la práctica 4:");
		tituloSeleccion.setBounds(50, 40, 300, 25); // (x, y, ancho, alto)
		panel.add(tituloSeleccion); 


		JComboBox<String> selectorModo = new JComboBox<String>(); 

		// Ponemos las opciones que se verán
		selectorModo.setModel(new DefaultComboBoxModel<String>(new String[] {
		    "Apartado 2", 
		    "Apartado 3", 
		    "Apartado 4",
		    "Apartado 5"
		...
		selectorModo.addActionListener(new ActionListener() {
		    public void actionPerformed(ActionEvent e) {
		        try { 
		            switch (selectorModo.getSelectedIndex()) { 
		                case 0: // Corresponde a Apartado 2
		                    commSerie.writeStringSerial("#2");
		                    break;

		                case 1: // Corresponde a Apartado 3
		                    commSerie.writeStringSerial("#3");
		                    break;

		                case 2: // Corresponde a Apartado 4
		                    commSerie.writeStringSerial("#4");
		                    break;
		                case 3: // Corresponde a Apartado 5
		                	commSerie.writeStringSerial("#5");
		                	break;
		            }
		        } catch (SerialPortException error) {
		            // Si se desconecta el puerto saldrá un error
		            error.printStackTrace();
		        }
		    }		
		});

		selectorModo.setBounds(50, 75, 200, 30); 
		panel.add(selectorModo);
	}
