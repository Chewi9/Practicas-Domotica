...
DomoBoard::DomoBoard()
{
	...
	pinMode(PIR_P, INPUT);				//Pin Sensor de movimiento PIR
	...
	PIR.pin 			= PIR_P;
	PIR.valor 			= digitalRead(PIR_P);
	PIR.valor_Df 		= LOW;
	PIR.Activo 			= true;
	PIR.eSensor  		= S_DIGITAL;
	PIR.name			= F("Sensor PIR");
	...
	// No se añade listSensor para PIR porque se gestiona por el ModBus
}
