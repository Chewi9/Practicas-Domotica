...
import practicas_eldom.config.MB_Registers.MB_Analog_Registers;
...
public class DomoBoardGui extends JPanel implements Visualizer {
	...
	private 		Radial2Top			Pot1;
	private 		Radial2Top			Pot2;
	...
	public DomoBoardGui(String category, int address, CommTransport sn_Transport) {
		...
		ANAregs = new int [MB_Registers.MB_Analog_Registers.MB_ANA_REGS.getReg()];
		...
		panelPot.setBorder(new BevelBorder(BevelBorder.LOWERED, new Color(0, 0, 255), new Color(0, 255, 0), Color.BLUE, Color.MAGENTA));
		panelPot.setBounds(10, 260, 404, 200);
		add(panelPot);
		
		JLabel labelPot = new JLabel("POT");
		labelPot.setHorizontalAlignment(SwingConstants.CENTER);
		labelPot.setForeground(Color.RED);
		labelPot.setFont(new Font("Times New Roman", Font.PLAIN, 20));
		labelPot.setBounds(10, 4, 380, 18);
		panelPot.add(labelPot);
		
		JLabel lblPot1 = new JLabel("POT 1 (A3)");
		lblPot1.setHorizontalAlignment(SwingConstants.CENTER);
		lblPot1.setFont(new Font("Times New Roman", Font.PLAIN, 12));
		lblPot1.setBounds(20, 170, 150, 18);
		panelPot.add(lblPot1);
		
		Pot1 = new Radial2Top();
		Pot1.setMinValue(0);
		Pot1.setMaxValue(1023);
		Pot1.setValue(0);
		Pot1.setTitle("POT 1");
		Pot1.setBounds(10, 25, 180, 145);
		panelPot.add(Pot1);
		
		
		JLabel lblPot2 = new JLabel("POT 2 (A4)");
		lblPot2.setHorizontalAlignment(SwingConstants.CENTER);
		lblPot2.setFont(new Font("Times New Roman", Font.PLAIN, 12));
		lblPot2.setBounds(220, 170, 150, 18);
		panelPot.add(lblPot2);
		
		Pot2 = new Radial2Top();
		Pot2.setMinValue(0);
		Pot2.setMaxValue(1023);
		Pot2.setValue(0);
		Pot2.setTitle("POT 1");
		Pot2.setBounds(210, 25, 180, 145);
		panelPot.add(Pot2);
	...
	public void Actualize() {
		...
		if(stActualize) {	
			...
			buildModBus(address, Const_Modbus.READ_INPUT_REGISTERS, MB_Registers.MB_Analog_Registers.MB_POT1.getReg(), MB_Registers.MB_Analog_Registers.MB_ANA_REGS.getReg(), ANAregs);
	...
	public void UpdateElements(final ModBusEvent e){
		...
		case Const_Modbus.READ_INPUT_REGISTERS:
			for (int i = 0; i< nReg; i++) {
				int valor = e.getRegs()[i];
				
				switch(MB_Analog_Registers.values()[i]) {
