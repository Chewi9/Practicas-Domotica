 ...
 * set_BankDiscrete
 *
 *   Configure the register used to Digital Inputs (Discrete Inputs)
 */
// Sirve para leer en DRegs el estado de los botones, y detectar si están pulsados o no, y enviar esa información al ModBus
// Se ejecuta al principio para que acuerden en qué lugar de la memoria dejan y cogen los mensajes de los botones
void ModbusSlave::set_BankDiscrete(uint16_t *Regs, uint16_t lengthRegs){
	DRegs = Regs; // DRegs apuntará a la dirección de Regs, que es el array que se le pasa como argumento. De esta forma, cuando se modifique el valor de DRegs[i], también se modificará el valor de Regs[i], ya que ambos apuntan a la misma dirección de memoria.
	N_DRegs = lengthRegs; // N_DRegs guarda el tamaño del array Regs, que en este caso será de 3
}
...
*/
	...
	// Leer botones solo
	if (FC_READ_REG_DI == data[FUNC]){ // Indica el tipo de comando que se debe ejecutar, en este caso un 2, que es el comando de lectura de registros digitales de entrada (discrete inputs)

		regs_size = N_DRegs; // Busca que lleguen 3, BTN1, BTN2, BTNOpt
	...
	/* check quantity of registers */
	// Leer relé y triac
	if (FC_READ_COILS == data[FUNC]){ // Indica el tipo de comando que se debe ejecutar, en este caso un 1, que es el comando de lectura de registros digitales de salida (coils)

		regs_size = N_CRegs; // Busca que lleguen 2, RELE, TRIAC
		max_regs_num = MAX_READ_REGD;
...
int ModbusSlave::update()
{
	...
	switch (query[FUNC]) {
	case FC_READ_COILS:
	// Lee banco CRegs, contiene rele, triac y se envia al maestro.
		return read_discrete_registers(FC_READ_COILS, start_addr,query[REGS_L],CRegs);
		break;

	case FC_READ_REG_DI:
	// Lee banco DRegs, contiene el estado de los botones, y se envia al maestro.
