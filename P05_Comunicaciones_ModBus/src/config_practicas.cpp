...
void config_practica5(void){
	DEBUGLNF("Práctica 5 seleccionada");

	domoboard.clear_managerActuators();

	domoboard.BOTON1.managerActuators.clear();
	domoboard.BOTON2.managerActuators.clear();
	domoboard.BTN_OPT.managerActuators.clear();

	domoboard.BOTON1.SensorEvent = mb_conmutador_rele;
	domoboard.BOTON2.SensorEvent = mb_conmutador_triac;
}