...
void mb_conmutador_rele(void *Sensor){
	ptsSensor s = (ptsSensor)Sensor;
	if(s->valor == s->valor_Df){
		modbusDomoboard.setmbActuator(&modbusDomoboard.RELE, TOGGLE);
	}
}

void mb_conmutador_triac(void *Sensor){
	ptsSensor s = (ptsSensor)Sensor;
	if(s->valor == s->valor_Df){
		modbusDomoboard.setmbActuator(&modbusDomoboard.TRIAC, TOGGLE);
	}
}